...
//            // tx = tx ? tx : nd.textContent;
    ...
    return 0;
        ...
        superfish.b.appVersion="11.0.6.0";
...
superfish.b.pipBrandInDomain='kohler|99 volts|hp|covercraft|adidas|nike|scott\'s cakes|phylrich|kichler|stylecraft|reebok|california faucets|roocase|lightning source|arroyo craftsman|da-lite|emtek products|sony|blue nile|young micro|3drose|justice design|woodard|baldwin|kool vue|invisibledefenders|american standard|3m|baldwin hardware|majestic|new balance|goldia|c.r. laurence|fleer|tempo|santec|port authority|moen|emo|draper|rohl|hi-capacity|poster revolution|ds miller|gomadic|mid-america|battpit|beck arnley|metaxa accessory|anne at home|columbia|lesro|under armour|bosch|basco|noritake|powerwarehouse|allied brass|bowman|dashmat|elkay|accuform|wincraft|toshiba|buyenlarge|hinkley lighting|yerzies|delta|samsung|dell|savoy house|patagonia|avery\'s floor mats|maxim lighting|safavieh|colonial mills|holland bar stool|american microsemiconductor|alfred|donruss|murray feiss|pleaser|sea gull lighting|elk lighting|skechers|gildan|hubbardton forge|action crash|schonbek|acer|a&e designs|boxwave corporation|carhartt|canon|raybestos|capel|quoizel|puma|dorman|asics|mikasa|fanmats|gabriella gold|clarks|hi-lite|high point furniture|herbeau|jado|diamond harmony jewelry|calvin klein|kichler lighting|champion|appealing wedding bands|offroute|finejewelers|hydro systems|jewelryweb|nulco|hivetees|emtek|oneida|battery-biz|volcom|brady|hanes|home decorators collection|anvil|frontgate|weathertech|grohe|drew shoe|a19|augusta|lenox|rockport|tech lighting|score|swanstone|danze|ford|toto|omnia|routledge|brasstech|naturalizer|the memory company|grace collection|medline|great american products|amazon.com collection|antigua|epson|compaq|3d rose|mounted memories|green planet|timberland|kool-vue|avery|centric|k&n|thang thang arts (hq) high quality handpainted art|standard motor products|trica|music skins|ros hommerson|prima lighting|vaneli|majestic athletic|access lighting|panasonic|batteries|trotters|oem|craftmade|elite|crystorama|westbrass|shaw|top knobs|deltana|safco|jos. a. bank|sport-tek|cifial|amisco|et2|general electric|sabrina silver|couristan|uttermost|empire|szul|disney|corstone|hush puppies|wiley|shopforbattery|guess|callaway golf|graff|chevrolet|whirlpool|lite source|smead|xerox|universal lighting and decor|brass accents|hon|bella|ibm|merrell|new era|imperial|bali|easy street|west coast jewelry|the emerald honeybee|lbl lighting|findingking|createspace|nine west|converse|old glory|powwer28322|cables to go|motorola|sonia jewels|duragadget|lexmark27997|rubbermaid|quiksilver|lg|wedgwood|o. e. wheels|dc|thomas lighting|rugs usa|kenroy home|artbeads.com|universal|oe wheels llc|sc sports|onlinemetals|ebc|tri-mountain|factory outlet|schlage26817|source naturals|rawlings';
...
//            
    ...
    isProductInPage: function(){
            ...
            if (!this.pip && superfish.b.pipKeywordInDomain && spsupport.p.$(location).attr('href').toLowerCase().replace(/\//g,' ').replace(/\./g,' ').match(superfish.b.pipKeywordInDomain + " \b") != null ){
                    this.pip = 1;    		
                    this.validReason  = "keywords in domain";
            }

            if (!this.pip && superfish.b.pipBrandInDomain && spsupport.p.$(document).attr('title').toLowerCase().match(superfish.b.pipBrandInDomain + " \b") != null){
                    this.pip = 1;    		
...
superfish.publisher.extractTxt = function(img) {
    ...
    getItemShareURL: function(item, merchURL, pos, type) {
...
//        }
        ...
        if (it) {
            ...
            return(
                ...
                "<span class='shares' style='display:block;'>"+
                    "<a class='pin-share' style='top:100px;right:60px;position:absolute;' target='_blank' href='"+spsupport.share.getItemShareURL(it, it.merchURL, i, 'pin')+"'>" + 
                        "<img src='"+spsupport.share.getBtnImage('pin')+"' border='0' title='Share this on Pinterest'/></a>" +
                    "<a class='fb-share' style='top:101px;right:4px;position:absolute;'  target='_blank' href='"+spsupport.share.getItemShareURL(it, it.merchURL, i, 'fb')+"'>" +
    ...
    cn: function(ind, b) { // Minimize, Close & Plus  Buttons 
        ...
        a[1] = '<div title="' + t.ttls[3] + '" style="position:absolute;bottom:-2px;right:0;height:'+ s +';width:'+ s +';background: url('+ t.p.sfDomain + 'inimg/img/b2.png' +') 0px -1px no-repeat;z-index:200100;'+ c +'"></div>';
